...
# ============================================================
# LOGO HELPER — embed as base64 so it renders in HTML blocks
# ============================================================
...
# ============================================================
# GLOBAL STYLES  — Light sky-blue theme matching UNO Minda UI
# ============================================================
st.markdown("""
<style>
...
[data-baseweb="select"] * { background: var(--white) !important; color: var(--text-dark) !important; }
[data-baseweb="popover"] {
    background: var(--white) !important; border: 1px solid var(--border) !important;
    border-radius: var(--r-md) !important; box-shadow: var(--shadow) !important;
}
[data-baseweb="menu"] li { background: var(--white) !important; color: var(--text-dark) !important; }
[data-baseweb="menu"] li:hover { background: var(--sky-light) !important; }
...
.stNumberInput button {
    ...
    border: 1px solid var(--border) !important; color: var(--text-mid) !important;
    border-radius: 6px !important;
}
.stNumberInput button:hover { background: var(--sky-mid) !important; }
...
[data-testid="stExpander"] summary p { color: var(--text-mid) !important; }
...
hr { border: none !important; border-top: 1.5px solid rgba(14,165,233,0.20) !important; margin: 12px 0 !important; }
...
.stat-chip:hover { background: rgba(186,230,253,0.75); }
...
.suggest-chip {
    display: inline-block; background: rgba(255,255,255,0.80);
    border: 1.5px solid rgba(14,165,233,0.30); border-radius: 50px;
    padding: 8px 20px; font-size: 0.83rem; font-weight: 600; color: var(--text-mid);
    cursor: pointer; transition: all 0.18s; white-space: nowrap;
}
.suggest-chip:hover {
    background: var(--white); border-color: var(--sky-accent);
    box-shadow: 0 2px 10px rgba(3,105,161,0.15); color: var(--sky-dark);
}
...
def build_prompt(resume_text: str) -> str:
    schema = {
        "candidate": {"full_name": None, "department": None, "email": None, "phone": None,
                       "location": None, "headline": None, "summary": None,
                       "total_years_experience": None,
                       "links": {"linkedin": None, "github": None, "portfolio": None, "other": []}},
        "skills": {"core": [], "technical": [], "tools": [], "soft": [], "domains": []},
        "experience": [{"company": None, "job_title": None, "employment_type": None,
                         "location": None, "start_date": None, "end_date": None,
                         "is_current": None, "bullets": []}],
        "education":      [{"institution": None, "degree": None, "field_of_study": None,
                             "location": None, "start_date": None, "end_date": None,
                             "grade_or_gpa": None, "notes": []}],
        "projects":       [{"name": None, "role": None, "start_date": None, "end_date": None,
                             "description": None, "technologies": [], "links": []}],
        "certifications": [{"name": None, "issuer": None, "issue_date": None, "expiry_date": None,
        ...
        "metadata": {"source": "resume_pdf", "extraction_notes": [],
...
def _llm_call(messages, max_tokens=3000, temperature=0.0) -> str:
    ...
    resp    = bedrock_runtime.invoke_model(modelId=LLM_MODEL_ID, body=json.dumps(body).encode(),
...
def parse_resume_pdf_to_json(pdf_path: str, department: str,
                              original_filename: str = "") -> Optional[Dict[str, Any]]:
    resume_text = extract_text_from_pdf(pdf_path)
    if not resume_text:
        return None
    raw = _llm_call([{"role": "user", "content": [{"type": "text", "text": build_prompt(resume_text)}]}])
    try:
        data = try_parse_json(raw)
    except Exception:
        repair = (f"Return STRICT valid JSON ONLY per the schema.\n"
                  f"Invalid output:\n\"\"\"{raw}\"\"\"\nResume:\n\"\"\"{resume_text}\"\"\"")
        data   = try_parse_json(_llm_call([{"role": "user", "content": repair}]))
...
def _profile_txt(r):
    ...
    return (f"Department: {c.get('department','')}\nLocation: {c.get('location','')}\n"
            f"Experience: {c.get('total_years_experience','')} years\nSummary: {c.get('summary','')}")

def _skills_txt(r):
    s = r.get("skills",{}) or {}
    j = lambda x: "; ".join(str(i).strip() for i in (x or []) if str(i).strip())
    return f"Core: {j(s.get('core'))}\nTechnical: {j(s.get('technical'))}\nTools: {j(s.get('tools'))}"

def _exp_txt(r):
    rows = []
    for e in (r.get("experience",[]) or []):
        rows.append(f"Role: {e.get('job_title','')} | Co: {e.get('company','')} | "
...
def upsert_resume(resume_json):
    ...
    client.upsert(collection_name=COLLECTION, points=[qmodels.PointStruct(
        id=pid,
        vector={"v_profile":    bedrock_embed(_profile_txt(resume_json)),
...
# ============================================================
def find_resume_pdf(full_name: str, original_filename: str) -> Optional[bytes]:
    rd = Path(RESUMES_DIR)
    if not rd.exists():
        return None
    if original_filename:
        p = rd / original_filename
        if p.exists():
            return p.read_bytes()
    if full_name:
...
def simple_tokenize(text: str) -> List[str]:
    stop = {"and","or","the","to","of","in","for","with","a","an","is","are","on",
            "as","at","by","be","we","you","our","will","shall","from","this","that","it"}
    return [t for t in re.findall(r"[A-Za-z0-9\+\#\.\-]{2,}", (text or "").lower()) if t not in stop]
...
def split_jd_4views(jd_text: str) -> Dict[str, str]:
    prompt = (f"Given this Job Description return STRICT JSON with keys:\n"
              f'"profile_view","skills_view","responsibilities_view","education_view"\n'
              f'Extract ONLY what is present.\n\nJD:\n"""{jd_text}"""')
    raw = _llm_call([{"role":"user","content":prompt}], max_tokens=900)
    try:
        out = json.loads(raw)
        return {k: out.get(k,"") for k in ("profile_view","skills_view",
                                            "responsibilities_view","education_view")}
    except Exception:
...
def qdrant_query(vec_name, vec, limit, q_filter=None):
    if hasattr(client, "query_points"):
        res = client.query_points(collection_name=COLLECTION, query=vec, using=vec_name,
                                   limit=limit, with_payload=True, query_filter=q_filter)
        return res.points
    return client.search(collection_name=COLLECTION, query_vector=(vec_name, vec),
                         limit=limit, with_payload=True, query_filter=q_filter)

def build_filter(min_years, dept, loc):
    must = []
    if min_years:
        must.append(qmodels.FieldCondition(key="total_years_experience",
                                            range=qmodels.Range(gte=min_years)))
    if dept.strip():
        must.append(qmodels.FieldCondition(key="department",
                                            match=qmodels.MatchText(text=dept.strip())))
    if loc.strip():
        must.append(qmodels.FieldCondition(key="location",
...
def extract_evidence_by_company(jd_text: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    matched_skills = [s for s in skills_flat
    ...
    for exp in experience_json:
        ...
        if matched_bullets:
            ...
            company_evidence[company].append(
                {"job_title": job_title, "matched_bullets": matched_bullets}
            )
...
def search_candidates_from_jd(
    ...
    def add_results(results, weight, key):
        for r in results:
            pid = str(r.id)
            if pid not in fused:
                fused[pid] = {"id": pid, "score": 0.0,
                              "payload": r.payload, "scores": {}}
...
def post_sort_results(results: List[Dict[str, Any]], jd_text: str,
                      sort_mode: str) -> List[Dict[str, Any]]:
...
def render_candidate_card(idx: int, result: Dict[str, Any], jd_text: str):
    ...
    score    = result["score"]                                    # direct fused score
    ...
    with st.expander(f"🎯  View Full Profile — {name}", expanded=(idx == 1)):
        ...
        pdf_bytes = find_resume_pdf(name, orig_fn)
        dl_col, _, _ = st.columns([1, 2, 1])
        with dl_col:
            if pdf_bytes:
                dl_name = (orig_fn if orig_fn.lower().endswith(".pdf")
                           else f"{name.replace(' ','_')}_resume.pdf")
                st.download_button(
                    label="📥  Download Resume PDF",
                    data=pdf_bytes, file_name=dl_name, mime="application/pdf",
                    key=f"dl_{idx}_{result['id']}", use_container_width=True,
                )
            else:
                st.info("💡  Resume PDF not found in Resumes/ folder")
        ...
        # ── Left: matched skills ─────────────────────────────
        with col_l:
            st.markdown(f"""<div class="inner-sec">
                <h5>🎯 Matching Skills ({skc})</h5></div>""",
                unsafe_allow_html=True)
...
def process_uploaded_resumes(uploaded_files, department: str) -> Tuple[int, int, int, List[str]]:
    ...
    for uf in uploaded_files:
        ...
        with open(resumes_dir / fn, "wb") as f: f.write(data)
        saved.append((path, fn))
    ...
    def worker(pdf_path: str, orig_fn: str):
        data = parse_resume_pdf_to_json(pdf_path, department=department,
                                         original_filename=orig_fn)
        if data is None: return ("no_text", None)
        return ("ok", upsert_resume(data))

    total, completed = len(saved), 0
    with ThreadPoolExecutor(max_workers=max(1, min(MAX_WORKERS, total))) as ex:
        futures = {ex.submit(worker, p, fn): (p, fn) for p, fn in saved}
        for fut in as_completed(futures):
            completed += 1
            try:
                tag, pid = fut.result()
                if tag == "ok":
                    ok += 1; upserted.append(pid)
                else:
                    no_text += 1
            except Exception:
                failed += 1
            progress.progress(completed / total,
                               text=f"Processing {completed}/{total} resumes…")
            status.markdown(f"✅ Indexed: **{ok}** &nbsp;|&nbsp; ⚠️ No text: **{no_text}** "
                            f"&nbsp;|&nbsp; ❌ Failed: **{failed}**")
...
with st.sidebar:
    ...
    if st.button("⚡  Process & Index Resumes", use_container_width=True, key="proc_btn"):
        if not uploaded_files:
            ...
            if ok:      st.success(f"✅ Indexed {ok} resume(s)!")
            if no_text: st.warning(f"⚠️ {no_text} had no text.")
            if failed:  st.error(f"❌ {failed} failed.")

    st.markdown("<hr>", unsafe_allow_html=True)

    # Filters
    st.markdown("#### 🔧 Search Filters")
    prefer    = st.selectbox("Weighting Strategy",
                              ["Balanced","Skills-relevant","Experience-relevant"])
    sort_mode = st.selectbox("Sort Results By",
                              ["Default (Relevance)",
...
# ── Main header: title only (no logo on right/main area) ───
st.markdown("""
...
# st.markdown("""
# <div style="padding:14px 0 20px 0;">
#     <p style="margin:0 0 10px 0;font-size:0.82rem;font-weight:700;color:#0369a1;
#               text-transform:uppercase;letter-spacing:0.6px;">💡 Try asking:</p>
#     <div style="display:flex;flex-wrap:wrap;gap:10px;">
#         <span class="suggest-chip">🏭 Quality Manager – Automotive</span>
#         <span class="suggest-chip">🔧 Plant Head – Manufacturing</span>
#         <span class="suggest-chip">📊 20+ years experience</span>
#         <span class="suggest-chip">🎯 IATF / ISO Certified</span>
#         <span class="suggest-chip">🚗 Auto Component Industry</span>
#     </div>
# </div>
# """, unsafe_allow_html=True)
...
if search_btn:
    ...
    q_filter = build_filter(
        ...
        dept_contains, loc_contains,
    )

    with st.spinner("🔍  Analysing job description and matching candidates…"):
        results, views = search_candidates_from_jd(
            jd_text=jd_text, top_k=top_k, per_vector_limit=120,
            prefer=prefer, q_filter=q_filter,
        )
    ...
    if results:
        ...
        """, unsafe_allow_html=True)
        st.markdown("""<h3 style="color:#0c2340;font-family:'Plus Jakarta Sans',sans-serif;
                    font-weight:800;font-size:1.2rem;margin-bottom:12px;">
                    👥&nbsp; Shortlisted Candidates</h3>""", unsafe_allow_html=True)
